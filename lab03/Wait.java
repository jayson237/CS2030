class Wait extends Event {
    ...
    public Pair<Event, ImList<Server>> execute(ImList<Server> serverList) {
        Server updated = new Server(this.server.getServerId(),
                this.server.getEndTime() + this.getCustomer().getServiceTime(), 
                this.server.getQueue(), this.server.getQMax());
        return new Pair<>(new Serve(this.server.getEndTime(), 
                this.getCustomer(), updated), serverList);
    }
