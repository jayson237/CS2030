...
class Simulator {
    ...
    Simulator(int numOfServers, int qmax, ImList<Double> arrivalTimes, Supplier<Double> serviceTime) {
    ...
    private ImList<Server> initServers(int numOfServers) {
        ...
        for (int i = 0; i < numOfServers; i++) {
            serverList = serverList.add(new Server((i + 1), 0, new ImList<Wait>(), this.qmax));
        }
        return serverList;
    }
    ...
    public String simulate() {
        ...
        averageWaiting /= waitEvent;
        int notServedCustomer = this.arrivalTimes.size() - servedCustomer;
        String counter = String.format("[%.3f %d %d]", averageWaiting, servedCustomer,notServedCustomer);
